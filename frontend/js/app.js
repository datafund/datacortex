...
class Datacortex {
    ...
    findNodeByTitle(query) {
        if (!this.graph || !this.graph.nodes) return null;
        const q = query.toLowerCase();
        // Exact match first
        let node = this.graph.nodes.find(n => n.title.toLowerCase() === q);
        if (node) return node;
        // Partial match
        node = this.graph.nodes.find(n => n.title.toLowerCase().includes(q));
        return node;
    }

    async findPath(sourceQuery, targetQuery) {
        const result = document.getElementById('path-result');

        // Try to resolve titles to node IDs
        let sourceId = sourceQuery;
        let targetId = targetQuery;

        // Check if input looks like a title (no slashes) vs an ID
        if (!sourceQuery.includes('/')) {
            const sourceNode = this.findNodeByTitle(sourceQuery);
            if (sourceNode) {
                sourceId = sourceNode.id;
                console.log(`Resolved "${sourceQuery}" to ${sourceId}`);
            } else {
                result.textContent = `Source "${sourceQuery}" not found`;
                return;
            }
        }

        if (!targetQuery.includes('/')) {
            const targetNode = this.findNodeByTitle(targetQuery);
            if (targetNode) {
                targetId = targetNode.id;
                console.log(`Resolved "${targetQuery}" to ${targetId}`);
            } else {
                result.textContent = `Target "${targetQuery}" not found`;
                return;
            }
        }

        try {
            const response = await fetch(`${API_BASE}/graph/path/${encodeURIComponent(sourceId)}/${encodeURIComponent(targetId)}`);
            const data = await response.json();
            ...
            result.textContent = 'Error finding path';
        }
    }
